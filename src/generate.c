 ...
 * See the "room_template.txt" file for more on room templates.
 *
...
static bool alloc_object(struct cave *c, int set, int typ, int depth, byte origin)
{
	...
	switch (typ) {
		...
		case TYP_TRAP: pick_and_place_trap(c, y, x, depth); break;
...
static void vault_trap_aux(struct cave *c, int y, int x, int yd, int xd)
{
	...
	for (tries = 0; tries <= 5; tries++) {
		...
		pick_and_place_trap(c, y1, x1, c->depth);
		break;
	}
}
		...
		for (dx = 0; dx < xmax && *t; dx++, t++) {
			...
			switch (*t) {
				...
				case '^': {
					if (randint0(100) < 25)
						/* Place a deep trap */
						pick_and_place_trap(c, y, x, c->depth + 10); 
					break;
				}
				case '*': {
					/* Treasure or a trap */
					if (randint0(100) < 75)
						...
						pick_and_place_trap(c, y, x, c->depth + 5);
					break;
				}
			}
